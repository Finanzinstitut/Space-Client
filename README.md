...
- **The Space Client companion mod** is installed automatically into every Fabric and Quilt instance running **Minecraft 26.2** — the create-instance dialog says plainly, in red, when the chosen combination cannot load it, including imported modpacks, and refreshed from its own GitHub releases. Each instance has a toggle to opt out. Forge and NeoForge instances skip it, since it is a Fabric mod.
...
- **Mod repair**: puts every installed mod onto a version that actually fits the instance. This is different from the update check, which only looks for something *newer* — a mod pinned to the wrong Minecraft version stays wrong and Fabric refuses to start. Repair swaps in whichever version matches, in either direction, and moves anything with no compatible version into an `incompatible` folder rather than deleting it. It runs automatically after importing a NoRisk pack, and there is a button for it in the Mods tab.
- **NoRisk packs**: the profile lists every entry under `mods`, whether it is a mod, a resource pack or a shader, without saying which. Space Client asks Modrinth in bulk to classify them so each file lands in the right folder. Entries disabled in the pack are written with a `.disabled` suffix. The pack's custom JVM arguments are not imported — there is no per-instance JVM argument field yet.
  ...
  Packs pin exact mod versions, and those are frequently not the versions the target Minecraft version needs, which is why an imported pack can leave the game unable to start. The repair step above runs automatically at the end of the import for exactly this reason.
